"""
source_proc_executor.py — Execute source DB stored procedures + functions.

Supports MSSQL, MySQL, MariaDB, and Oracle via source_adapter.SourceAdapter.
...
Required env vars: SOURCE_TYPE (default: mssql), SOURCE_HOST, SOURCE_USER,
                   SOURCE_PASSWORD, SOURCE_DATABASE
Legacy aliases still accepted: MSSQL_HOST, MSSQL_USER, MSSQL_PASSWORD, MSSQL_DATABASE
Optional env vars: SOURCE_PORT, VALIDATION_OUTPUT_DIR, VALIDATION_SKIP_WRITES,
...
from config import (MSSQL_CONF, MSSQL_OUTPUT_FILE, SHARED_PARAMS_FILE,
                    ...
                    is_mssql_system_schema, check_required, SOURCE_TYPE)
from source_adapter import build_adapter
from param_discovery import sample_mssql_params

# Output file uses source-agnostic name; keep MSSQL_OUTPUT_FILE as alias
SOURCE_OUTPUT_FILE = MSSQL_OUTPUT_FILE
...
def discover_schemas():
    ...
    return _adapter.get_schemas()

# ── Procedure + Function discovery ───────────────────────────────────────────
def get_all_procs(schema):
    """Return dict of name_lower -> {name, proc_def, params, obj_kind} for a schema."""
    routines = _adapter.get_routines(schema)
    result = {}
    for rinfo in routines:
        proc_name_raw = rinfo['name']
        obj_type      = rinfo['type']       # 'P' or 'FN'
        obj_type_desc = rinfo['type_desc']

        params  = _adapter.get_parameters(schema, proc_name_raw)
        proc_def = _adapter.get_routine_body(schema, proc_name_raw) or ''

        kind_map = {'P': 'PROCEDURE', 'FN': 'FUNCTION',
                    'IF': 'FUNCTION', 'TF': 'FUNCTION'}
        obj_kind = kind_map.get(obj_type, obj_type_desc or 'PROCEDURE')
        ...
        result[proc_name_raw.lower()] = {
            ...
            'params':   [{'name':      p['name'],
                          'type':      p['type_name'],
                          'is_output': p['is_output']} for p in params],
        }
    return result
...
def main():
    ...
    with open(SOURCE_OUTPUT_FILE, 'w', encoding='utf-8') as f:
        for rec in records:
    ...
    print("\n" + "=" * 65)
    print("SOURCE DONE [%s]  Success=%d  Errors=%d  Untestable=%d  Skipped=%d" % (
          SOURCE_TYPE.upper(),
          success, errors, untestable, skipped))
    print("Output: %s" % SOURCE_OUTPUT_FILE)
    print("=" * 65)
    return records
