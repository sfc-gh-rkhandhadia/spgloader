"""
validate_batch.py — View validation: source DB vs Snowflake Postgres.

Supports MSSQL, MySQL, MariaDB, and Oracle source databases via SOURCE_TYPE env var.
Dynamically discovers all views in both databases.
No hardcoded schema names, view names, or connection details.

Required env vars: SOURCE_TYPE (default: mssql), SOURCE_HOST (or MSSQL_HOST),
                   SOURCE_USER (or MSSQL_USER), SOURCE_PASSWORD, SOURCE_DATABASE,
...
from config import (MSSQL_CONF, SPG_CONF, BATCH_SIZE, OUTPUT_DIR,
                    ...
                    check_required, SOURCE_TYPE)
from source_adapter import build_adapter as _build_adapter
import psycopg2
import reporting as rpt

_src_adapter = _build_adapter()

def _src_conn(): return _src_adapter.connect()
...
def discover_mssql_views():
    """Return all (schema, view_name) pairs from source DB, excluding system schemas."""
    conn = _src_conn()
    cur = conn.cursor()
...
def query_mssql_view(schema, view_name):
    try:
        ...
        if SOURCE_TYPE == 'mssql':
            ...
            cur.execute(f'SELECT COUNT(*) FROM "{schema}"."{view_name}"')
            count = cur.fetchone()[0]
            cur.execute(f'SELECT * FROM "{schema}"."{view_name}" WHERE 1=0')
